#!/usr/bin/env python3
"""
Scrape ELK Studios slots from gamingslots.com
...
print("="*60)
print("Scraping ELK Studios slots from gamingslots.com")
print("="*60)
print()
...
try:
    ...
    # Find all slot links
    slot_links = soup.find_all('a', href=re.compile(r'/slots/elk-studios/[^/]+-slot/'))
    ...
    for i, slot_url in enumerate(sorted(unique_urls), 1):
        ...
        match = re.search(r'/slots/elk-studios/([^/]+)-slot/', slot_url)
        if not match:
            continue
        ...
        try:
            ...
            # Save image
            filename = f"elkstudios-{slug}{ext}"
    ...
    with open('elk_studios_slots.json', 'w') as f:
        json.dump(slots, f, indent=2)
    
    print(f"\n✓ Saved {len(slots)} slots to elk_studios_slots.json")
