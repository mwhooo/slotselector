#!/usr/bin/env python3
"""
Scrape Kalamba Games slots from gamingslots.com
"""
...
print("="*60)
print("Scraping Kalamba Games slots from gamingslots.com")
print("="*60)
print()
...
try:
    ...
    # Find all slot links
    slot_links = soup.find_all('a', href=re.compile(r'/slots/kalamba-games/[^/]+-slot/'))
    ...
    for i, slot_url in enumerate(sorted(unique_urls), 1):
        match = re.search(r'/slots/kalamba-games/([^/]+)-slot/', slot_url)
        if not match:
            continue
        ...
        try:
            ...
            filename = f"kalambagames-{slug}{ext}"
    ...
    with open('kalamba_games_slots.json', 'w') as f:
        json.dump(slots, f, indent=2)
    
    print(f"\n✓ Saved {len(slots)} slots to kalamba_games_slots.json")
