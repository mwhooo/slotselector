"""
Scrape NoLimit City slots from gamingslots.com
Note: gamingslots.com only has ~34 NoLimit City slots available
"""
import requests
...
BASE_URL = "https://www.gamingslots.com/slots/nolimitcity/"
HEADERS = {
...
print("=" * 60)
print("Scraping NoLimit City slots from gamingslots.com")
print("=" * 60)
...
for i, url in enumerate(sorted(unique_urls), 1):
    ...
    slot_name = re.sub(r'\bXways\b', 'xWays', slot_name, flags=re.IGNORECASE)
    slot_name = re.sub(r'\bXnudge\b', 'xNudge', slot_name, flags=re.IGNORECASE)
    slot_name = re.sub(r'\bXbomb\b', 'xBomb', slot_name, flags=re.IGNORECASE)
    ...
    try:
        ...
        filename = f"nolimitcity-{slot_slug}{ext}"
        filepath = os.path.join(OUTPUT_DIR, filename)
        
        # Check if already exists
        existing = [f for f in os.listdir(OUTPUT_DIR) if f.startswith(f"nolimitcity-{slot_slug}.")]
        if existing:
