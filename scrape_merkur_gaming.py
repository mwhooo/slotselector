#!/usr/bin/env python3
"""
Scrape Merkur Gaming slots from gamingslots.com
"""
...
url = "https://www.gamingslots.com/slots/merkur-gaming/"
...
print("="*60)
print("Scraping Merkur Gaming slots from gamingslots.com")
print("="*60)
print()
...
try:
    ...
    # Find all slot links
    slot_links = soup.find_all('a', href=re.compile(r'/slots/merkur-gaming/[^/]+-slot/'))
    ...
    for i, slot_url in enumerate(sorted(unique_urls), 1):
        match = re.search(r'/slots/merkur-gaming/([^/]+)-slot/', slot_url)
        if not match:
            continue
        ...
        try:
            ...
            filename = f"merkurgaming-{slug}{ext}"
    ...
    with open('merkur_gaming_slots.json', 'w') as f:
        json.dump(slots, f, indent=2)
    
    print(f"\n✓ Saved {len(slots)} slots to merkur_gaming_slots.json")
