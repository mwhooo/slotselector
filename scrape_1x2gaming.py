#!/usr/bin/env python3
"""
Scrape 1x2gaming slots from gamingslots.com
...
url = "https://www.gamingslots.com/slots/1x2gaming/"

print("="*60)
print("Scraping 1x2gaming slots from gamingslots.com")
print("="*60)
print()
...
try:
    ...
    # Find all slot links
    slot_links = soup.find_all('a', href=re.compile(r'/slots/1x2gaming/[^/]+-slot/'))
    ...
    for i, link in enumerate(slot_links, 1):
        href = link.get('href')
        match = re.search(r'/slots/1x2gaming/([^/]+)-slot/', href)
    ...
    # Save to JSON file
    with open('1x2gaming_slots.json', 'w') as f:
        json.dump(list(slots.keys()), f, indent=2)
    
    print(f"\n✓ Saved {len(slots)} slots to 1x2gaming_slots.json")
