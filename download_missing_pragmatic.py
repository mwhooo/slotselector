#!/usr/bin/env python3
"""Download missing Pragmatic Play slot images from gamingslots.com"""
import json
...
# Get only Pragmatic Play slots
pragmatic_slots = {name: provider for name, provider in slots.items() if provider == 'Pragmatic Play'}

print(f"Downloading missing Pragmatic Play slot images...")
print(f"Total Pragmatic slots: {len(pragmatic_slots)}\n")
...
for i, slot_name in enumerate(pragmatic_slots.keys(), 1):
    ...
    try:
        if i % 20 == 0:
            print(f"[{i}/{len(pragmatic_slots)}] Processing {slot_name}...", flush=True)
...
print(f"TOTAL DOWNLOADED: {successful}/{len(pragmatic_slots)}")
