#!/usr/bin/env python3
"""
Scrape Backseat Gaming slots from gamingslots.com
- Scrape slot names and download images in one pass
"""
...
print("="*60)
print("Scraping Backseat Gaming slots from gamingslots.com")
print("="*60)
print()
...
try:
    ...
    # Find all slot links
    slot_links = soup.find_all('a', href=re.compile(r'/slots/backseat-gaming/[^/]+-slot/'))
    ...
    for i, slot_url in enumerate(sorted(unique_urls), 1):
        ...
        match = re.search(r'/slots/backseat-gaming/([^/]+)-slot/', slot_url)
        if not match:
            continue
        ...
        try:
            # Fetch slot page
            slot_response = requests.get(slot_url, timeout=10)
            ...
            # Save image
            filename = f"backseatgaming-{slug}{ext}"
    ...
    with open('backseat_gaming_slots.json', 'w') as f:
        json.dump(slots, f, indent=2)
    
    print(f"\n✓ Saved {len(slots)} slots to backseat_gaming_slots.json")
